...
class timer{
  ...
  Future<void> cleartimer(Box<datalist> hivedata) async {
   ...
   var data=hivedata.values.toList().where((element) 
    =>
      (jsonDecode(element.CaptData)['Date'])==DateFormat('dd/MM/yyyy').format(DateTime(DateTime.now().year,DateTime.now().month,DateTime.now().day-expiron))
);
 data.forEach((element) {if(element.id!=0)element.delete();});
  }
}
